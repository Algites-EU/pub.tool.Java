...
import static eu.algites.tool.build.model.artifact.intf.AInArtifactClass.UNDEFINED;
...
import eu.algites.tool.build.model.artifact.intf.AIiAbstractDefinedArtifact;
...
/**
 * <p>
 * Title: {@link AIcArtifactModelLoadingContainer}
 * </p>
 * <p>
 ...
 * @date 10.01.26 14:50
 */
public class AIcArtifactModelLoadingContainer<A extends AIiAbstractArtifact> implements AIiArtifactCoordinate {

	private final String coordinateId;
	private final String groupId;
	private final String artifactId;

	private final Map<String, AIcArtifactVersionModelLoadingContainer<A>>
			internalArtifactVersions = new HashMap<>();
	private final Map<String, AIcArtifactVersionModelLoadingContainer<A>>
			publicArtifactVersions = Collections.unmodifiableMap(internalArtifactVersions);

	AIcArtifactModelLoadingContainer(final AIiArtifactCoordinate aArtifactCoordinate) {
	 ...
	 */
	protected Map<String, AIcArtifactVersionModelLoadingContainer<A>> getInternalArtifactVersions() {
	 ...
	 */
	public Map<String, AIcArtifactVersionModelLoadingContainer<A>> getArtifactVersions() {
	 ...
	 */
	public AIcArtifactVersionModelLoadingContainer<A> getUnspecifiedVersionContainer() {
		...
		if (!aExistingArtifact.getDefinedOutputs().containsAll(aUndefinedArtifactToCheck.getDefinedOutputs())) {
			...
			try {
				...
				String locControlledArtifactConfigurationFileName
						= aExistingArtifact instanceof AIiAbstractControlledArtifact
						? ((AIiAbstractControlledArtifact) aExistingArtifact).getArtifactConfigurationFile().toString() : null;
	 ...
	 */
	public void putUndefinedArtifact(final AIcArtifactVersionModelLoadingContainer<AIiUndefinedArtifact> aArtifactContainer,
			Supplier<String> aHumanReadableOperationContextInfoSupplier) throws IllegalArgumentException {
		AIcArtifactVersionModelLoadingContainer<? extends AIiAbstractArtifact> locExistingArtifactContainer
		...
		if (locExistingArtifactContainer == null)
			getInternalArtifactVersions().put(aArtifactContainer.getArtifact().getNormalizedArtifactVersion(),
					(AIcArtifactVersionModelLoadingContainer<A>) aArtifactContainer);
		else {
	...
	public void putUncontrolledArtifact(final AIcArtifactVersionModelLoadingContainer<AIiAbstractUncontrolledArtifact> aArtifactContainer,
			Supplier<String> aHumanReadableOperationContextInfoSupplier) {
		AIcArtifactVersionModelLoadingContainer<A> locExistingArtifactContainer
				= getInternalArtifactVersions().get(aArtifactContainer.getArtifact().getNormalizedArtifactVersion());
		if (locExistingArtifactContainer == null)
			getInternalArtifactVersions().put(aArtifactContainer.getArtifact().getNormalizedArtifactVersion(),
					(AIcArtifactVersionModelLoadingContainer<A>) aArtifactContainer);
			...
			case CONTROLLED:
					throw new IllegalArgumentException("\\\\ Development error: The definition already exists as a controlled artifact and cannot be overwritten by the uncontrolled artifact. "
							...
							+ "defined in the file '" + ((AIcArtifactVersionModelLoadingContainer<AIiAbstractControlledArtifact>) locExistingArtifactContainer).getArtifact().getArtifactConfigurationFile()
	 ...
	 * @throws IllegalArgumentException If some user or development error occurs.
	 */
	public void putControlledArtifact(final AIcArtifactVersionModelLoadingContainer<AIiAbstractControlledArtifact> aArtifactContainer,
			Supplier<String> aHumanReadableOperationContextInfoSupplier) {
		AIcArtifactVersionModelLoadingContainer<A> locExistingArtifactContainer = getInternalArtifactVersions().get(UNSPECIFIED_VERSION_PLACEHOLDER);
		if (locExistingArtifactContainer == null)
			getInternalArtifactVersions().put(UNSPECIFIED_VERSION_PLACEHOLDER, (AIcArtifactVersionModelLoadingContainer<A>) aArtifactContainer);
		else
			switch (locExistingArtifactContainer.getArtifact().getArtifactClass()) {
			case UNDEFINED:
				...
				break;
			case CONTROLLED:
				if (Objects.equals(((AIiAbstractControlledArtifact)locExistingArtifactContainer.getArtifact()).getArtifactConfigurationFile(),
				...
				throw new IllegalArgumentException("Duplicate artifact definition. The artifact with coordinatedId '"
						+ aArtifactContainer.getArtifact().getCoordinateId()
						+ "' is defined in the file '" + ((AIiAbstractControlledArtifact)locExistingArtifactContainer.getArtifact()).getArtifactConfigurationFile()
