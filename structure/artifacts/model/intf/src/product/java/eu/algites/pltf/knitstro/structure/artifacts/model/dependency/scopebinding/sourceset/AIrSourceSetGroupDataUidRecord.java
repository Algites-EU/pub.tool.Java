...
/**
 * Parsed representation of the Source Sets UID.
 *
 ...
 * @param groupCode groupCode (must be not empty)
 * @author linhart1
 */
public record AIrSourceSetGroupDataUidRecord(
		String uid,
		AInUidEnumDataOrigin origin,
		String namespace,
		String groupCode
) implements AIiSourceSetGroupDataUidRecord {

	static {
		AIiSourceSetGroupDataUidRecord.staticValidation();
	}
	 ...
	 * @return the factory
	 */
	public static BiFunction<String, List<String>, AIrSourceSetGroupDataUidRecord> getUidRecordFactory() {
		return (aUid, aParts) -> {
			AInUidEnumDataOrigin locOrigin = AInUidEnumDataOrigin.getByCodeOrThrow(aParts.get(ORIGIN_UID_POSITION));
			return new AIrSourceSetGroupDataUidRecord(aUid,
					locOrigin,
