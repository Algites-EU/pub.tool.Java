 ...
 * @author linhart1
 */
public record AIrArtifactTemplateDataUidRecord(
		String uid,
		AInUidEnumDataOrigin origin,
		String namespace,
		String templateCode
) implements AIiArtifactTemplateDataUidRecord {

	static {
		AIiArtifactTemplateDataUidRecord.staticValidation();
	}
	 ...
	 * @return the factory
	 */
	public static BiFunction<String, List<String>, AIrArtifactTemplateDataUidRecord> getUidRecordFactory() {
		return (aUid, aParts) -> {
			AInUidEnumDataOrigin locOrigin = AInUidEnumDataOrigin.getByCodeOrThrow(aParts.get(ORIGIN_UID_POSITION));
			return new AIrArtifactTemplateDataUidRecord(aUid,
					locOrigin,
