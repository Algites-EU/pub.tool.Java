 ...
 *        {@link AInBuiltinDependencyScopeBindingTemplate#getAllowedForPurposeInDependencyWithArtifactKinds()}.<br/>
 ...
 * 		    If {@link AInBuiltinDependencyScopeBindingTemplate#IMPORT} is used, the dependency must be declared in MANAGED context only
 ...
 * 		    (Model hint: {@link AInBuiltinDependencyScopeBindingTemplate#isUsedForManagedDependenciesOnly()}.)</li>
 *
 * 		  <li><b>I-02 IMPORT requires POM projection (ERROR)</b><br/>
 * 		    If {@link AInBuiltinDependencyScopeBindingTemplate#IMPORT} is used, then {@link AIiArtifactOutput#getOutputPackagingId()} must be {@code "pom"}.</li>
 *
 * 		  <li><b>I-03 IMPORT requires an importable target (ERROR)</b><br/>
 * 		    If {@link AInBuiltinDependencyScopeBindingTemplate#IMPORT} is used, then the target kind must be an importable BOM kind,
 ...
 * 		    If {@link AInBuiltinDependencyScopeBindingTemplate#IMPORT} is used, then {@link AIiArtifactOutput#getOutputClassifier()} should be empty/null.
 ...
 * 		    If {@code target.kind == UNCONTROLLED_BOM} and {@link AIiArtifactDependencyScope#getLevel()} is not {@link AInBuiltinDependencyScopeBindingTemplate#IMPORT}, then:
 * 		    <ul>
 ...
 * 		    If {@link AIiArtifactDependencyScope#getLevel()} is {@link AInBuiltinDependencyScopeBindingTemplate#IMPORT}, then {@code target.kind != UNCONTROLLED_CORE}.</li>
 *
 ...
 * 		    If {@link AIiArtifactDependencyScope#getLevel()} is {@link AInBuiltinDependencyScopeBindingTemplate#UNDEFINED}, dependency must be declared only in MANAGED context.
 ...
 *
 * 		<p>
 * 		It is strongly recommended to interpret {@link AInBuiltinDependencyScopeBindingTemplate#getAllowedForPurposeInDependencyWithArtifactKinds()} on scope levels as constraints on the
